...
| enum| **[Content](#enum-content)** \{ Content_Colours, Content_BumpMap, Content_NormalMap, Content_AlphaTest \}<br />Values for Idx_Content.  |
| enum| **[Encoding](#enum-encoding)** \{ Encoding_RGB_XYZ, Encoding_RGB_XZY \}<br />Values for Idx_Encoding.  |
| enum| **[IDX](#enum-idx)** \{ Idx_Filename, Idx_Minify, Idx_Magnify, Idx_WrapU, Idx_WrapV, Idx_ColourSpace, Idx_DDSData, Idx_DDSModifiedTime, Idx_Content, Idx_Encoding, Idx_AnisotropicDegree, Idx_Transform, Idx_MapChannel, Idx_LODBias, Idx_AlphaRef \} |
| enum| **[Magnify](#enum-magnify)** \{ Magnify_None = 9728, Magnify_Bilinear = 9729 \}<br />Values for Idx_Magnify.  |
| enum| **[Minify](#enum-minify)** \{ Minify_None = 9728, Minify_Bilinear = 9729, Minify_Trilinear = 9987 \}<br />Values for Idx_Minify.  |
| enum| **[Wrap](#enum-wrap)** \{ Wrap_ClampWhite = 33069, Wrap_ClampEdge = 33071, Wrap_Repeat = 10497, Wrap_Mirrored = 33648 \}<br />Values for Idx_WrapU and Idx_WrapV.  |
...
The C API may also convert an arbitrary blob of pixel data to DDS using [VRBlobToDDSProperty](../../modules/core.mdx#vrblobtoddsproperty)
...
| Idx_ColourSpace | | [ColourSpace](#enum-colourspace) (int) - The colour space of the texture. A non-default value may involve automatic internal conversion to the colour space used by the application.   |
...
| Idx_Content | | [Content](#enum-content) (int) - The usage that this data is intended for   |
| Idx_Encoding | | [Encoding](#enum-encoding) (int) - The encoding of the data when it is intended to be used for normal mapping   |
