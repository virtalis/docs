...
| Type           | Name           |
| -------------- | -------------- |
| enum| **[CONSTRAINT_TYPE](#enum-constraint_type)** \{ CONSTRAINT_NONE, CONSTRAINT_LOCK, CONSTRAINT_PARENT, CONSTRAINT_LIMIT, CONSTRAINT_IK, CONSTRAINT_LOOKAT \}<br />Values for Idx_Constraint.  |
| enum| **[IDX](#enum-idx)** \{ Idx_Enabled, Idx_Transform, Idx_Constraint, Idx_ShadowCaster, Idx_CollisionLayer, Idx_SnapEnabled, Idx_IgnoreCoverageLOD, Idx_IgnoreProbes \}<br />Property index.  |
| enum| **[LAYERS](#enum-layers)** \{ LAYER_INVALID = INT_MIN, LAYER_DEFAULT = 0 \}<br />Values for Idx_CollisionLayer.  |
...
| Idx_Constraint | | [CONSTRAINT_TYPE](#enum-constraint_type) (int) - Type of constraint to be enforced on this assemblies transform.   |
