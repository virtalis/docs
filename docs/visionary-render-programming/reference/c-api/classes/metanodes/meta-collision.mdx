...
| Type           | Name           |
| -------------- | -------------- |
| const int | **[GROUP_CAMERA](#group-camera)** <br />defines a collision group for cameras  |
| const int | **[GROUP_DEFAULT](#group-default)** <br />defines a standard collision group; DEFAULT objects collide with everything other than PICKING and PICKABLE objects  |
| const int | **[GROUP_HELPERS](#group-helpers)** <br />defines a collision group for immersive helpers that require collision with the avatar but with not the picking rays  |
| const int | **[GROUP_PICKABLE](#group-pickable)** <br />defines a Mockup compatible collide group; PICKABLE objects only collide with PICKING objects  |
| const int | **[GROUP_PICKING](#group-picking)** <br />defines a Mockup compatible collide group; PICKING objects only collide with PICKABLE objects  |
| const int | **[GROUP_SYSTEM](#group-system)** <br />defines a collide group for the systems always on picking - Skinnies without a Fat sibling  |
| const int | **[GROUP_VISUAL](#group-visual)** <br />defines a collision group for Skinny bodies that have a Fat sibling - always the visual mesh  |
