...
The core of VRTree implements the Observer pattern to perform change subscription or notification. Node instances can be observed, meaning that callback functions are called in response to changes to the tree structure or other node data.
