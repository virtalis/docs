...
![](/img/programming-guide/material_texture_ref.png)
...
A `GeoGroup` can reference a `Material`, which is applied to all of its meshes. When a `Model` or `LOD` contains multiple `GeoGroup`s, each `GeoGroup` may reference a different material.

![](/img/programming-guide/model_structure.png)
...
![](/img/programming-guide/visual_model_ref.png)
