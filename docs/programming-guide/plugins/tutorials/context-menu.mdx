...
![](/img/programming-guide/context_menu_nodes.png)
...
This type of callback function does not receive any parameters. Instead it should read the registers set by the event system. For more details on the FFI, see @ref ffi_howto.
...
| Type                 | Purpose                                  |
| -------------------- | ---------------------------------------- |
| `TypeApp`            | Triggers an @ref advanced_appcommands "application command" specified in the `Command` property of the menu item. It may also pass a parameter specified by the `UserString` property of the menu item, if required. |
