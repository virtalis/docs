...
![](/img/programming-guide/message-dialog.png)

When a plugin is loaded, it is given a [pointer to a message box function](../../reference/c-api/modules/plugins/interface.mdx#vrpusermessageproc), which should be stored by the plugin and called when it wants to display a message.
...
![](/img/programming-guide/question-dialog.png)

When a plugin is loaded, it is given a [pointer to a question box function](../../reference/c-api/modules/plugins/interface.mdx#vrpuserquestionproc) and a [pointer to a response callback registration function](../../reference/c-api/modules/plugins/interface.mdx#vrpuserquestioncallbackregisterproc). The plugin is expected to store these, and to use the callback registration function prior to presenting the question in order to register a callback function which is called with the user's answer.
