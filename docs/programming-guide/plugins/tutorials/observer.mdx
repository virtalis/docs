...
- @ref NodeCreatedFunc "Node Creation"
- @ref NodeDestroyingFunc "Node Destruction"
- @ref NodeValuesChangedFunc "Node Property Changes"
- @ref NodeRenamedFunc "Node Name Changes"
- @ref NodeChildAddedFunc "Node Children Added"
- @ref NodeChildRemovedFunc "Node Children Removed"
- @ref NodeParentChangedFunc "Node Parent Changes"
...
A plugin may wish to implement these functions when it needs to respond to things such as property changes on @ref define_metanode_howto "metanodes it has defined".
...
This is discussed more fully in @ref advanced_metanode_versioning.
...
This means, for example, that a ::NodeValuesChangedFunc can read property values, but it should not set them. Instead it should defer those property write operations until the next VRTree Update using an @ref update_howto "Update callback".
...
The @ref best_practices page describes the difference in handle behaviour between those requested and those provided to callback functions.
