---
title: Meta_LightProbe
summary: A LightProbe represents a local skybox providing dynamic reflections of the surrounding environment. 
...
A LightProbe represents a local skybox providing dynamic reflections of the surrounding environment.  [More...](#detailed-description)
...
|                | Name           |
| -------------- | -------------- |
| enum| **[IDX](../Classes/struct_meta___light_probe.mdx#enum-idx)** { Idx_Enabled, Idx_Resolution, Idx_IncludeNode, Idx_ExcludeNode, Idx_ProxyType, Idx_ProxySize, Idx_Shadows, Idx_UpdateMode, Idx_UpdateAndStoreNow, Idx_Texture, Idx_Mode, Idx_BakedTexture, Idx_ViewBoundsNode}<br />Property index.  |
| enum| **[ProbeMode](../Classes/struct_meta___light_probe.mdx#enum-probemode)** { Probe_Bake, Probe_Custom}<br />Values for Idx_Mode.  |
| enum| **[ProxyType](../Classes/struct_meta___light_probe.mdx#enum-proxytype)** { Proxy_None, Proxy_Ellipsoid, Proxy_Box}<br />Values for Idx_ProxyType.  |
| enum| **[UpdateMode](../Classes/struct_meta___light_probe.mdx#enum-updatemode)** { Update_Auto, Update_Always, Update_Manual}<br />Values for Idx_UpdateMode.  |
...
These are positioned in the scene by adding them as a child of an [Assembly](../Classes/struct_meta___assembly.mdx) node and transforming that. 
...
| Idx_IncludeNode | | link - A node in the scene that should render into the light probe (defaults to the root scene, in other words 'everything').   |
| Idx_ExcludeNode | | link - A node in the scene that should _not_ render into the light probe (defaults to null, in other words 'nothing').   |
...
| Idx_ViewBoundsNode | | link - Defines the bounds within which the light probe should be used   |
