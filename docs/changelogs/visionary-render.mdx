...
- A tree view dialog to permit settings to be selected for saving and loading
- Command-line option `-restoresettings` to merge a settings file immediately after the user settings are loaded
- Support for showing images in tooltips
...
- Prevent the view in render to texture nodes (such as the mirror in the start room) from looking too flat in HMDs or with stereo rendering enabled
...
- LuaSQL can throw an error message in scripts
