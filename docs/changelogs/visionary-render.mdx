...
- Support for Pico Motion Trackers in OpenXR mode
- Ability to take control of manikins via the immersive context menu
- Wizard to permit body tracking to be calibrated
- Ability to change the current audio device without having to restart Visionary Render for changes to take effect
- Image stabilisation and zoom settings for headset output
- Improve the appearance of shadows by supporting Horizon-Based Ambient Occlusion (NVIDIA HBAO+)
...
- Render to Texture does not work
- Joystick positions and buttons are lost when the Oculus runtime is started in OpenXR mode
...
- Stabilised HMD view disappears when scene scale is small
