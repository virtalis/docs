...
- `vrControlManikinTargets` function to permit a specified user (or local user if none specified) to possess a manikin
- `vrReleaseMankin` function to permit a manikin to be released from a user (either the user or manikin can be passed as an argument)
...
- Missing Lua observer callbacks to `vrAddNodeObserver` and `vrAddMetaNodeObserver` to support better node event tracking and bring the Lua API in line with the C API
- `vrNodeGetUUID` function to get the UUID of a node
- `vrCompressTexture` function to apply DXT compression to a texture
- `vrCanCompressTexture` function to determine whether DXT compression can be applied to a texture
- `vrScaleTexture` function to scale a texture by a specified factor
- `vrCanScaleTexture` function to determine whether a texture can be scaled
...
- `vrBakeMesh` function to convert deformable meshes to static meshes for export and upload
- `vrGetDirectoryContents` function to return the files and folders in a directory as a table of tables
- Missing documentation for various optional parameters

#### Fixed

- `vrShowGUIFrame` ignores its `show` parameter and can't be used to hide the GUI
