...
- Missing Lua observer callbacks (`nodeChildAdded`, `nodeChildRemoved`, `nodePathChanged`, `nodeValuesChanged`, `nodeLinkAdded`, `nodeLinkRemoved`, and `nodeCloned`) to `vrAddNodeObserver` and `vrAddMetaNodeObserver` to support better node event tracking and bring the Lua API in line with the C API
