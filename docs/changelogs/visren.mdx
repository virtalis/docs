...
- Adds the ability to upload your scenes to Virtalis Hub using the App Menu > Upload to Virtalis Hub... option
- Adds support for new server types such as RTSP to Movie nodes
- Adds an alternative connection mode for the HoloLens 2 that permits the headset to connect to the PC using a QR code
- Adds the ability to download scenes from Virtalis Hub directly to Visionary Render
- Adds support for the XR_FB_passthrough extension in OpenXR, which permits passthrough to be enabled when using the Quest Link in developer mode. Also adds adjustable background opacity in AR

#### Changed

- Improves save speed for scenes with a very large number of models in a library
- Improves the speed of rendering particle systems in stereo

#### Fixed

- Stops executing new custom action frames by default when sequences are looped. Note that you should use property tracks instead if possible
- Fixes issue where the whole scene could disappear when changing World Scale via the controllers in a headset
- Fixes issues with occlusion culling that could lead to flickering geometry
- Fixes issue where scene GUIs could have rendering artifacts after loading
- Fixes a crash that could occur when toggling the Edit Pivot mode button
- Fixes issue with the message `OpenXR lists 0 API layers` being repeated in the log
- Fixes issue with the file browser not being able to follow shortcuts
- Fixes issue with UI settings and window positions not being restored after restarting Visionary Render
- Makes measuring points visible in the Scenes tree view and fixes issue with the parent object being selected when clicking on a measuring point in the 3D scene
...
- Fixes a crash that could occur when rendering the first frame of a complex scene
- Fixes issue where the HoloLens settings button didn't always show the correct tab in the settings window
- Fixes issue where occlusion culling failed to cull occluded objects that are visible in only one eye when rendering in stereo
- Fixes issue with the auto-hide mouse setting not working when hovering over scene GUIs
- Fixes issue with the tracking configuration buttons being incorrectly greyed out at startup
...
- Adds support for QR code tracking to the HoloLens 2 to permit scenes and objects to be aligned to real-world locations
...
- Fixes fly-to so that it does not include the bounds of disabled objects  #13589 - Fix the Iray degrain settings so that options that are not relevant to the current filter mode are not displayed
- Fixes issues with scroll-to-zoom on the mouse navigator forcing the character controller through walls  #13868 - Fixes undo when undoing the addition of Iray MDL materials
...
- Rolls back fix #9841 due to side-effects seen in 1.3.3
