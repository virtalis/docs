...
- Support for URLs in borehole datasets as per Shapefiles
- Min/max scale, offset property (Z offset), and depth test toggle to the Billboard metanode
- Silent installs to permit admins to quickly install, update and remove GeoVisionary via central management systems

#### Removed

- The CAD importers (Division, Drawing and Design) are no longer bundled with GeoVisionary but can be installed separately if required

#### Fixed

- Exporting a shape file produces a file with no data
- Spatial reference/EPSG Codes do not always give the location
- Gradients are not being applied to Styles and Shapefiles
- Gradients on a Voxel using the Smooth Iso Mode option look incorrect
- Scaling a point cloud doesn't affect the scale of the visual, but does affect the bounding box
...
- Ability to install the full set of Visionary Render importers in GeoVisionary

#### Changed

- Update voxel renderer to CUDA 13 

#### Fixed

- Installers can be run on an x86 platform
