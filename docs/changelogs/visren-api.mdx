...
- Added `vrSplitVirtalisHubURL` for identifying Virtalis Hub URLs and splitting out their server name, artifact ID and artifact name

#### Fixed

- Makes `vrDot` work with `vrVec4` values
...
- Added `vrGenerateQRCodeFile` to permit QR codes to be generated and saved to a file (which is used by the HoloLens 2 to view scenes in Augmented Reality)
...
- Fixed incorrect return values from `vrNodeIsInGroup`
...
- Added a performance warning regarding `vrNodeGetChildCount` and `vrNodeGetChildByIndex` to the developer guide
...
- Fixed logging so that failures in the plugin sandbox can be understood
...
- Fixed an occasional crash after certain combinations of Lua script errors
...
- `vrNodeFindChild` now returns `nil` instead of the node to search from when given an empty search path
...
- Added 2 functions to the Lua API for observing events:
...
- Added 32 functions to the C API for observing events:
...
- Added 14 functions to the Lua API for creating and editing user settings:
...
- Permits `vrAddPropertyObserver` to observe multiple properties of a metanode
- Improves `vrNodeHasTrait(node:hasTrait())` to support checking for traits without version numbers and without the `Trait_` prefix. For example, `n:hasTrait("AttributeTable")` as a shortcut for `n:hasTrait("Trait_AttributeTable.1")`

#### Deprecated

- Deprecates passing non-function objects as callbacks to `__registerCallback` and adds a warning message when it happens

#### Fixed

- Fixes various instances where infinite progress displays could be spawned from Lua scripts
- Fixes issue where you cannot register Lua functions in the Lua state provided by `vrtree.dll`
- Fixes issue with the developer guide start menu shortcut not being uninstalled
- Prevents crashes when passing a vector or matrix to a Lua function that expects a node
- Prevents scripts in the StandardActions library from being run multiple times in a collaboration
...
- Improves the `vrAddPropertyObserver` Lua function to allow multiple properties to be observed by a single observer
...
- Added 40 functions to the Lua API for for creating your own metanode types:
...
- Added 40 functions to the C API for creating and observing settings:
...
- Fixes an issue with `vrSelectNodes({})` no longer working
...
- Adds Lua API function `vrGetHoldingUser` to determine which user has grabbed an object

#### Changed

- Improves the Getting Started section of the developer guide
...
- `VRGetPrev` Gets the previous sibling of a node
- `VRGetPrevOfType` Gets the previous sibling of a specific type
- `VRSetParentEx` Sets the parent of a node, specifying a sibling to insert after

#### Deprecated

- Marked `vrSetTrackedHandNode` and `vrSetTrackedEyesNode` as deprecated
...
- Makes various minor corrections to the developer guide
...
- C API function `VRGetPropertyEx()` which lets you get the property index from a metanode name and property name
- `VRRemoveCallbackEx` variants added to allow you to remove callbacks specific to a piece of user data
...
- The `vrGeometryOp()` Lua function now supports `optimise`, `split`, `explode`, `prune`, `sieve`, `recentre`, `transform` and `merge` operations

#### Fixed

- Fixes crash that could occur when compressing DDS pixel data in the API
- Corrects the documentation for various Lua API functions
...
- New method introduced to be able to get the metanode handle from a given node handle
  - `VRGetMetaNode`

- Adds API methods to support working with properties containing vectors of strings
...
- Adds API methods to access fixed sized floating point value arrays such as those used for matrices
...
- Adds section to developer guide demonstrating pulling in data from external web resources
...
- DEV-1140 - Removes `Enabled` property from all nodes under the toolbox. Any scripts that refer to the `Enabled` property of a tool node should be updated to refer to the relevant setting instead
...
- Adds plugin SDK and documentation to the application installation in `data/vrtreesdk`, with a start menu shortcut for the developer guide
...
- Adds an improved API for node observer in Lua. Updates the function signatures of `vrAddNodeObserver` and `vrAddMetaNodeObserver`. The old versions have been deprecated. See [Observers](../visren-dev/scripts/metanodes/observers.mdx) for more information
...
- Adds the [VR Exchange API](../visren-dev/reference/c-api/modules/vrexchange.mdx)
...
- Adds more built in Lua documentation and auto-complete information
- Adds optional fade effects to `vrBodyFlyTo`
- Adds support for custom surface shaders

#### Changed

- Improves the developer documentation for users who implement complex scripts, by providing a supplementary, searchable developer guide covering events, plugins, and shaders, as well as architectural overviews and advanced topics
...
- Removes deprecated Lua functions `vrNodeGetWorldMatrix` and `vrNodeSetWorldMatrix`
...
![](/img/visren-dev/diagnostics-window.png)
