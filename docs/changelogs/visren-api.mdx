...
- Adds `vrSplitVirtalisHubURL` for identifying Virtalis Hub URLs and splitting out their server name, artifact ID and artifact name
...
- FUS-1745 - 40 functions to the Lua API for for creating your own metanode types:
...
- FUS-1228 - 40 functions to the C API for creating and observing settings:
...
- The vrGeometryOp() Lua function now supports further operations:
  - optimise
  - split
  - explode
  - prune
...
- Corrects documentation for Lua API functions:
  - vrEnableWhenIrayAvailable
  - vrClusterMasterNode
...
- New metanode for vector strings and setter getter properties functionality
...
- New methods to set and get matrix data. Users now can pass default matrix values
...
- Updates the function signatures of vrAddNodeObserver and vrAddMetaNodeObserver. The old versions have been deprecated. See [Observers](scripts/metanodes/observers.mdx) for more information
...
- Adds the [VR Exchange API](reference/c-api/modules/vrexchange.mdx)
...
- @subpage v13x_200_lua_co
- @subpage v13x_200_lua_observers
