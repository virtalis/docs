...
- License manager sometimes cannot connect to server when it is updated whilst using a floating license
...
- Full user interface and usability refresh

#### Fixed

- Correct subscription and maintenance licence entitlements are not always applied at startup
...
- "Start licence server on startup" option as modern Windows systems are better managed through service configuration
- Changelog (this has been moved to Virtalis Docs)
- Unit test data in the `data/tests` directory

#### Fixed

- Prevent multiple instances from running at once and show a warning if the server isn't run as a service
...
- Sign the installer for the 2019.4 release of the license server
...
- Improve error reporting from the license server
