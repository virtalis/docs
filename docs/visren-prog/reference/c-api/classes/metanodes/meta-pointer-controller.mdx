...
| Type           | Name           |
| -------------- | -------------- |
| enum| **[Hand](#enum-hand)** \{ Left, Right \}<br />Values for Idx_Hand.  |
| enum| **[IDX](#enum-idx)** \{ Idx_Enabled, Idx_Transform, Idx_Camera, Idx_ButtonSource, Idx_TouchSource, Idx_ShowCursor, Idx_ShowLaser, Idx_LaserSelectEnabled, Idx_Length, Idx_VisibleToAll, Idx_VisibleToSelf, Idx_DoubleClickDistanceTolerance, Idx_DoubleClickAngleTolerance, Idx_Hand, Idx_ActionMask \} |
...
| Idx_ButtonSource | | link - Optionally specify a [Meta_Camera](../metanodes/meta-camera.mdx) which should be the only one that can see the pointer   |
...
| Idx_ActionMask | | [Hand](../metanodes/meta-pointer-controller.mdx#enum-hand) (int) - Which hand should this be treated as? Button event codes are adjusted based on handedness.   |
