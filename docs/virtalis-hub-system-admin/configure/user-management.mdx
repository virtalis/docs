...
This section describes how to manage Virtalis Hub user details, creating user groups and add selected users to the groups.

Virtalis Hub uses Keycloak for Identity and Access Management (IAM). 
...
Navigate to `https://SITE/auth/admin/` replacing 'SITE' with the domain Virtalis Hub is hosted on.

Enter the Keycloak administrator credentials that were extracted during the Virtalis Hub deployment.

Ensure that the currently selected realm in the top left corner is Virtalis Hub. If not, select it from the drop-down menu.
...
### Virtalis Hub Specific Groups

Virtalis Hub has three main system groups:
...
- hub_script_publishers - controls whether a user can enable scripts for their project
