...
| Advanced | Adjust these values to alter the size of a body segment by uniformly scaling the segment. The measurement is the length of each segment. The [Measuring Tool](#measuring-tool) can be used to verify anthropometric measurements on the manikin model when using **Advanced** Properties. |
