...
| 5 | The measuring points will appear in the tree view beneath the nodes they are attached to. They can also be shown in the Browser via the Measurements filter. |  |
...
| Display Mode | Dynamic – show measurements with an appropriate unit for their length (i.e. 1300mm would be shown as 1.3m). [default]Base Unit – show measurements with a single unit, as defined by the Base Unit setting. |
| Display Mode > Units | (This setting is shown when the Display Mode is set to Dynamic.)Metric – show measurements in millimetres, metres, and kilometres. [default]Imperial – show measurements in inches, feet, and miles. |
| Display Mode > Base Unit | (This setting is shown when the Display Mode is set to Base Unit.)A single unit to show measurements in. There are 8 options:Millimetres (mm)Centimetres (cm)Metres (m) [default]Kilometres (km)Inches (in)Feet (ft)Yards (yd)Miles (mi) |
...
| Snap To Vertex | By default, snapping only occurs when the Alt key is held down whilst dragging a measuring point. To enable it without having to press Alt, click Tools > Measuring > Snap To > Vertex in the Settings window. Pressing Alt will now disable snapping instead.The colour and size of the vertex markers can be configured from the Settings window under User Interface > Mouse > Snap.[default: false] |
