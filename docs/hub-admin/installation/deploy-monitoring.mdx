...
This section covers the deployment of various monitoring services which allow a Kubernetes Administrator to monitor the health, metrics, and logs for all cluster services including Reach.
...
Navigate to the default Reach installation folder
...
export REACH_VERSION=$(kubectl get secret reach-version \
-n $REACH_NAMESPACE -o json \
...
./install-reach.sh
kubectl label namespace $REACH_NAMESPACE prometheus-scrape=true
```
